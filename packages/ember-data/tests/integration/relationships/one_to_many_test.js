...
test("Rollbacking a record works correctly when the belongsTo side has been edited - async", function () {
  ...
  run(function() {
    ...
    user.get('messages').then(function(fetchedMessages) {
      equal(fetchedMessages.get('length'), 5, 'User does not have the message anymore');
      deepEqual(fetchedMessages.toArray(), [message1,message2,message3,message4,message5], 'User only has the original messages');
    });
  });
});
